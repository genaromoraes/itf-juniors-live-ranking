...
const POINTS_TABLE_FILE = path.join(OUT_DIR_CONFIG, "junior_points_table.csv");
...
export function resolvePaths(args = parseArgs()) {
  ...
  const outputDir = args.outputDir ? path.resolve(args.outputDir) : DEFAULT_OUT_DIR_CLEAN;

  return {
    inputDir,
    outputDir,
    weekPlayerResultsFile: path.join(inputDir, "week_player_results.csv"),
    weekMatchesFile: path.join(inputDir, "week_matches.csv"),
    weekLivePointsFile: path.join(outputDir, "week_live_points.csv"),
    weekLiveLedgerRowsFile: path.join(outputDir, "week_live_ledger_rows.csv"),
    pointsTableFile: POINTS_TABLE_FILE,
  };
}
...
async function ensurePointsTableExists(pointsTableFile) {
  try {
    await fs.access(pointsTableFile);
    return;
  } catch {
    const rows = getDefaultPointsRows();

    await writeCsv(pointsTableFile, rows, [
      "category",
      "event_type",
...
export function getLivePoints(row, roundLabel, pointsMap) {
  ...
  const key = [category, eventType, eventClassification, roundLabel].join("|");
...
export function buildLivePointRows(playerResults, matchRows, pointsMap) {
  ...
  return playerResults.map((row) => {
    ...
    return {
      ...
      live_points_weighted: eventType === "doubles" ? doublesWeightedPoints : livePoints,
...
function buildLedgerRows(livePointRows) {
  return livePointRows
    ...
    .map((row) => ({
      player_id: row.player_id,
      player_name: row.player_name,
      gender: row.player_type_code === "B" ? "M" : row.player_type_code === "G" ? "F" : "",
...
function printSummary(livePointRows, ledgerRows) {
  ...
  const withPoints = livePointRows.filter((row) => toNumber(row.live_points_raw) > 0).length;
  ...
  for (const item of [...byTournament.values()].sort((a, b) =>
    ...
    console.log(
      `${item.tournament_name}: ${item.with_points}/${item.rows} com pontos | maximo ${item.max_points}`
    );
  }
}
...
export async function main(cliArgs = parseArgs()) {
  ...
  await ensurePointsTableExists(paths.pointsTableFile);

  console.log("");
  console.log("Lendo resultados da semana...");
  const playerResults = await readCsv(paths.weekPlayerResultsFile);

  console.log("Lendo partidas da semana...");
  const matchRows = await readCsv(paths.weekMatchesFile);

  console.log("Lendo tabela de pontos...");
  const pointsRows = await readCsv(paths.pointsTableFile);
  ...
  await writeCsv(paths.weekLiveLedgerRowsFile, ledgerRows, [
    "player_id",
    "player_name",
    "gender",
    "country",
  ...
  console.log(paths.weekLivePointsFile);
  console.log(paths.weekLiveLedgerRowsFile);
  console.log(paths.pointsTableFile);
}
